...
func part1(input string) int {
	add := 0 + 0i
	return solve(input, add)
}

func part2(input string) int {
	add := 10000000000000 + 10000000000000i
	return solve(input, add)
}

func solve(input string, add complex128) int {
	var inputs []Input
	var b Input
	var i int
	for _, row := range strings.Split(input, "\n") {
		...
		switch i {
		case 0:
			b.ButtonA = parseDigits(row)
		case 1:
			b.ButtonB = parseDigits(row)
		default:
			b.Prize = parseDigits(row)
			b.Prize += add
		}
		i++
	}
	...
	for _, i := range inputs {
		num := real(i.Prize)*imag(i.ButtonA) - imag(i.Prize)*real(i.ButtonA)
		den := real(i.ButtonB)*imag(i.ButtonA) - imag(i.ButtonB)*real(i.ButtonA)
		b := num / den
		a := (real(i.Prize) - real(i.ButtonB)*b) / real(i.ButtonA)
...
func parseDigits(str string) complex128 {
	matches := digits.FindAllStringSubmatch(str, -1)
	real := utils.ToInt(matches[0][1])
	imag := utils.ToInt(matches[1][1])
